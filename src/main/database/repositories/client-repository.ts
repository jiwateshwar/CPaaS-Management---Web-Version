...
export class ClientRepository extends BaseRepository {
  list(params: ClientListParams): PaginatedResult<Client> {
    let where = 'WHERE 1=1';
    const qp: unknown[] = [];
  ...
  getById(id: number): Client | null {
    return (
      (this.db
        ...
        .get(id) as Client | undefined) ?? null
    );
  }

  getByCode(code: string): Client | null {
    return (
      (this.db
        .prepare('SELECT * FROM clients WHERE code = ?')
        .get(code) as Client | undefined) ?? null
    );
  }
  ...
  create(dto: CreateClientDto): Client {
    const result = this.db
      .prepare(
        `INSERT INTO clients (name, code, contact_name, contact_email, contact_phone, billing_currency, payment_terms, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        dto.name,
        ...
        dto.billing_currency ?? 'USD',
        dto.payment_terms ?? null,
        dto.notes ?? null,
      );
  ...
  update(dto: UpdateClientDto): Client {
    const existing = this.getById(dto.id);
    if (!existing) throw new Error(`Client ${dto.id} not found`);
    ...
    if (dto.billing_currency !== undefined) { fields.push('billing_currency = ?'); values.push(dto.billing_currency); }
    if (dto.payment_terms !== undefined) { fields.push('payment_terms = ?'); values.push(dto.payment_terms); }
