...
export class RoutingRepository extends BaseRepository {
  list(params: RoutingListParams): PaginatedResult<RoutingAssignment> {
    let where = 'WHERE 1=1';
    const qp: unknown[] = [];
    ...
    return this.paginate<RoutingAssignment>(
      baseQuery,
      countQuery,
      { ...params, sortBy: params.sortBy || 'ra.effective_from' },
      qp,
    );
  }
  ...
  create(dto: CreateRoutingDto): RoutingAssignment {
    ...
    const assignment = this.db
      ...
      .get(Number(result.lastInsertRowid)) as RoutingAssignment;
