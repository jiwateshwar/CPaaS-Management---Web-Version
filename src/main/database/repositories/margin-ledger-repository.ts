...
export class MarginLedgerRepository extends BaseRepository {
  // NO update method - ledger is immutable
  // NO delete method - ledger is immutable

  insert(entry: Omit<MarginLedgerEntry, 'id' | 'calculated_at' | 'locked'>): MarginLedgerEntry {
    const result = this.db
      .prepare(
        `INSERT INTO margin_ledger (
          traffic_record_id, client_id, vendor_id, country_code,
          channel, use_case, traffic_date, setup_count, monthly_count, mt_count, mo_count, message_count,
          vendor_rate_id, vendor_rate, vendor_setup_fee, vendor_monthly_fee, vendor_mt_fee, vendor_mo_fee, vendor_currency, vendor_cost,
          client_rate_id, client_rate, client_setup_fee, client_monthly_fee, client_mt_fee, client_mo_fee, client_currency, client_revenue,
      ...
      .run(
        ...
        entry.mo_count,
        entry.message_count,
        entry.vendor_rate_id,
        entry.vendor_rate,
        entry.vendor_setup_fee,
        entry.vendor_monthly_fee,
        entry.vendor_mt_fee,
        entry.vendor_mo_fee,
        entry.vendor_currency,
        entry.vendor_cost,
        entry.client_rate_id,
        entry.client_rate,
        entry.client_setup_fee,
        entry.client_monthly_fee,
        entry.client_mt_fee,
        entry.client_mo_fee,
        entry.client_currency,
        entry.client_revenue,
        entry.fx_rate_id,
        entry.fx_rate,
        entry.normalized_vendor_cost,
  ...
  reverseEntry(originalEntryId: number, reason: string): MarginLedgerEntry {
    const original = this.getById(originalEntryId);
    if (!original) throw new Error(`Ledger entry ${originalEntryId} not found`);
    if (original.is_reversal) throw new Error('Cannot reverse a reversal entry');

    // Check if already reversed
    const existing = this.db
      .prepare(
        'SELECT id FROM margin_ledger WHERE original_entry_id = ? AND is_reversal = 1',
      )
      .get(originalEntryId) as { id: number } | undefined;

    if (existing) {
      throw new Error(
        `Entry ${originalEntryId} already reversed by entry ${existing.id}`,
      );
    }
    ...
    return this.insert({
      ...
      setup_count: -original.setup_count,
      monthly_count: -original.monthly_count,
      mt_count: -original.mt_count,
      mo_count: -original.mo_count,
      message_count: -original.message_count,
      vendor_rate_id: original.vendor_rate_id,
      vendor_rate: original.vendor_rate,
      vendor_setup_fee: original.vendor_setup_fee,
      vendor_monthly_fee: original.vendor_monthly_fee,
      vendor_mt_fee: original.vendor_mt_fee,
      vendor_mo_fee: original.vendor_mo_fee,
      ...
      client_setup_fee: original.client_setup_fee,
      client_monthly_fee: original.client_monthly_fee,
      client_mt_fee: original.client_mt_fee,
      client_mo_fee: original.client_mo_fee,
  ...
  getById(id: number): MarginLedgerEntry | null {
    return (
      (this.db
        .prepare(
        ...
        .get(id) as MarginLedgerEntry | undefined) ?? null
    );
  }

  list(params: LedgerListParams): PaginatedResult<MarginLedgerEntry> {
    let where = 'WHERE 1=1';
    const qp: unknown[] = [];
    ...
    return this.paginate<MarginLedgerEntry>(
      baseQuery,
      countQuery,
      { ...params, sortBy: params.sortBy || 'ml.traffic_date' },
      qp,
    );
  }
}
