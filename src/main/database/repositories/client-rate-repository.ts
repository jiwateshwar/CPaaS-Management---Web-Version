...
export class ClientRateRepository extends BaseRepository {
  ...
  list(params: ClientRateListParams): PaginatedResult<ClientRate> {
    let where = 'WHERE 1=1';
    const qp: unknown[] = [];
    ...
    return this.paginate<ClientRate>(
      baseQuery,
      countQuery,
      { ...params, sortBy: params.sortBy || 'cr.effective_from' },
      qp,
    );
  }

  getById(id: number): ClientRate | null {
    return (
      (this.db
        .prepare(
          `SELECT cr.*, c.name as client_name, cm.name as country_name
        ...
        .get(id) as ClientRate | undefined) ?? null
    );
  }
  ...
  getEffective(
    ...
    return (
      (this.db
        ...
        .get(clientId, countryCode, channel, useCase, date, date) as
        | ClientRate
        | undefined) ?? null
    );
  }

  insertWithVersioning(dto: CreateClientRateDto): ClientRate {
    return this.db.transaction(() => {
      ...
      const overlaps = this.db
        .prepare(
          `SELECT id, effective_from, effective_to FROM client_rates
           WHERE client_id = ? AND country_code = ? AND channel = ? AND use_case = ?
             AND effective_from < ?
             AND (effective_to IS NULL OR effective_to > ?)`,
        )
